...
public class Board {
 ...
 public boolean isOccupied(Coord c) {
	 return (this.getValue(c) != 0 && this.getValue(c) != -1);
 }
 ...
 /*public Board deepCopy() {
	 Map<Coord, Integer> new_grid = new HashMap<Coord, Integer>();
	 for(Coord c: this.grid.keySet()) {
		 new_grid.put(c, this.grid.get(c));
	 }
	 Board new_board = new Board(new_grid, this.height, this.width);
	 return new_board;
 }*/
 ...
 public void pretty_print_grid(){
  ...
  for (int y=0; y != this.height; ++y){
   ...
   for (int x=0; x != this.width; ++x){
    int v = this.grid.get(new Coord(y, x));
